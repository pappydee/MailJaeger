...
class TestSingleActionApplySecurity:
    ...
    def test_missing_apply_token_returns_409_no_status_change(self, client, auth_headers):
    ...
    def test_invalid_apply_token_returns_409(self, client, auth_headers):
        ...
        with patch('src.main.get_db') as mock_get_db:
            ...
            mock_db.query.return_value.filter.return_value.first.side_effect = [None, mock_action]
    ...
    def test_move_folder_to_non_allowlisted_fails_without_imap(self, client, auth_headers):
        """Test that MOVE_FOLDER to non-allowlisted folder fails WITHOUT IMAP connection"""
        with patch('src.main.get_db') as mock_get_db, \
             patch('src.main.IMAPService') as mock_imap_service:
