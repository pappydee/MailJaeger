...
class TestFailClosedAuthentication:
    ...
    def test_dashboard_requires_auth_when_no_api_key(self, client):
        ...
        assert (
            response.status_code == 401
        ), "Dashboard should return 401 when no API key configured"
    ...
    def test_settings_requires_auth_when_no_api_key(self, client):
        ...
        assert (
            response.status_code == 401
        ), "Settings should return 401 when no API key configured"
    ...
    def test_email_list_requires_auth_when_no_api_key(self, client):
        ...
        assert (
            response.status_code == 401
        ), "Email list should return 401 when no API key configured"
    ...
    def test_settings_does_not_expose_imap_username(self):
        ...
        with patch.dict(
            ...
            with patch("src.main.get_db") as mock_db:
                ...
                assert (
                    "imap_username" not in settings_data
                ), "IMAP username should not be exposed"
...
class TestProductionComposeSecurity:
    ...
    def test_prod_compose_does_not_expose_ollama_port(self):
        ...
        assert (
            "11434:11434" not in prod_compose or "# DO NOT expose ports" in prod_compose
