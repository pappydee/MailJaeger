...
class TestFailClosedWebExposedPolicy:
    ...
    def test_web_exposed_blocks_safe_mode_false_and_require_approval_false_with_0_0_0_0(
        self,
    ):
    ...
    def test_web_exposed_blocks_safe_mode_false_and_require_approval_false_with_trust_proxy(
        self,
    ):
    ...
    def test_web_exposed_blocks_safe_mode_false_and_require_approval_false_with_allowed_hosts(
        self,
    ):
...
class TestSanitizeErrorPreventsSecretLeakage:
    ...
    def test_sanitize_error_prevents_secret_leakage_when_debug_false(self):
        ...
        exc = ValueError(
            "Database error: password=secret123 user=test@example.com host=db.internal.com"
        )
    ...
    def test_processing_run_error_message_redacts_secrets_in_debug_mode(self):
        ...
        run = ProcessingRun(
            ...
            error_message=sanitize_error(
                ValueError(
                    "Connection failed: password=secret123 user=test@example.com"
                ),
                debug=True,
            ),
            completed_at=datetime.utcnow(),
